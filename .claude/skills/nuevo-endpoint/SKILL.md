---
name: nuevo-endpoint
description: Checklist obligatoria para agregar o modificar un endpoint en este API de gimnasio (Spring Boot) que toque autenticación, roles, dinero, o datos de otro usuario. Usar antes de escribir código cuando el pedido sea "agregar un endpoint", "agregar un rol", "dejar que X haga Y", o cualquier cambio a SecurityConfig/controllers/DTOs de request.
---

# Nuevo endpoint / cambio de autorización

Este skill existe porque el feature de login de clientes se implementó sin
este chequeo y dejó dos huecos de autorización en producción (un CLIENTE
podía auto-activarse sin pagar, y un refresh token filtrado servía como
access token por 30 días). Ver `specs/AUTHZ-MATRIX.md` § Historial de
incidentes para el detalle. La causa raíz en ambos casos fue la misma: se
agregó un endpoint/rol nuevo sin releer explícitamente la matriz de permisos
completa.
...
7. **Validación:** agregar anotaciones `jakarta.validation` al DTO de
   request, siguiendo el patrón existente en `LoginRequest`,
   `ClienteRegistroRequest`, etc.

8. **Tests:** agregar un caso de integración por rol relevante, incluyendo el
   caso "un CLIENTE (o el rol con menos privilegios que exista) intenta
   acceder al recurso de otro" esperando 403/404 según corresponda.
...
9. **Antes de terminar:**
   ...
   - Si se tocó `SecurityConfig`, un controller, o un DTO de request, correr
     `/security-review` sobre el diff antes de dar el cambio por terminado.
